...
- 유저가 결제한 금액이나 특정 이벤트에 따라 포인트를 적립하고, 적립한 포인트로 상품을 결제할 수 있습니다.
...
1. **일반 포인트 적립**
    - **구매 보상**:
        - 결제 금액의 3%를 포인트로 적립.
        - 단, **최소 결제 금액은 1000원 이상**이어야 함.
        - 예) 1,000원 결제 시 30포인트 적립.
    - **일일 로그인 보상**:
        - 매일 로그인 시 **10포인트** 적립.
    - **운영 보상**:
        - 서비스 장애나 불편으로 인해 운영 측에서 최대 **1,000포인트** 지급.
2. **포인트 유효 기간**
    - 적립된 포인트는 **6개월 내**에 사용해야 하며, 이후 자동 소멸.
3. **포인트 회수**
    - **구매 취소 시** 적립된 포인트는 회수.
    - 단, 이미 사용한 포인트는 차감하지 않음.
4. **포인트 사용 제한**
    - **결제 금액의 50% 이상은 재화 결제**여야 함.

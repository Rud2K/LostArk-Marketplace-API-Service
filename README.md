...
- 로스트아크 오픈 API의 Market 정보를 실시간으로 반영하면서도 불필요한 API 호출을 줄이기 위해 Redis Cache와 Batch 작업을 조합해 사용합니다.

**세부 기능**

- **Redis Cache 저장**:
    - 로스트아크 오픈 API에서 불러온 Market 정보를 Redis에 일정 시간(5분) 동안 캐시.
- **Batch 작업**:
    - Spring Scheduler로 5분마다 Batch 작업을 실행하여 Market 정보를 최신 상태로 갱신.
- **Cache 만료 시 API 호출**:
